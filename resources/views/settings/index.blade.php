            ...
            <div class="panel">
                ...
                <div class="page-content settings container-fluid">
                    <ul class="nav nav-tabs">
                        @foreach($settingTypes as $group => $settingType)
                    ...
                    <div class="tab-content">
                        @foreach($settingTypes as $group => $group_setting_types)
                        @php $group_settings = $settings[$group]; @endphp
                        <div id="{{ \Illuminate\Support\Str::slug($group) }}" class="tab-pane fade in @if($group == $active) active @endif">
                            @foreach($group_setting_types as $key => $settingType)
                            ...
                            <div class="panel-heading">
                                ...
                                <div class="panel-actions">
                                    ...
                                    <i class="voyager-trash"
                                       ...
                                       data-display-name="{{ $settingType->display_name }}"></i>
                                    @endcan
                                </div>
                            </div>
                            ...
                            <div class="panel-body no-padding-left-right">
                                <div class="col-md-10 no-padding-left-right">
                                    ...
                                    @elseif($settingType->type == "radio_btn")
                                        ...
                                        <ul class="radio">
                                            @if(isset($options->options))
                                                @foreach($options->options as $index => $option)
                                                    <li>
                                                        <input type="radio" id="option-{{ $index }}" name="{{ $settingType->key }}"
                                    ...
                                    @elseif($settingType->type == "checkbox")
                                        ...
                                        @else
                                            <input type="checkbox" name="{{ $settingType->key }}" @if($checked) checked @endif class="toggleswitch">
                                        @endif
                                    @endif
                                </div>
                                <div class="col-md-2 no-padding-left-right">
                                    <select class="form-control group_select" name="{{ $settingType->key }}_group">
                                        @foreach($groups as $group)
                                        <option value="{{ $group }}" {!! $settingType->group == $group ? 'selected' : '' !!}>{{ $group }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
